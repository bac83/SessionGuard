...
public static class SessionGuardApiEndpoints
{
    public static IEndpointRouteBuilder MapSessionGuardApi(this IEndpointRouteBuilder app)
    {
    ...
    private static BadRequest<ApiErrorResponse> ValidationError(string message) =>
        TypedResults.BadRequest(new ApiErrorResponse("validation_failed", message));

    private static NotFound<ApiErrorResponse> NotFoundError(string message) =>
